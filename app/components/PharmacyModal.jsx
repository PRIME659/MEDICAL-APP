...
export default function PharmacyModal({ drug, onClose, onAddToCart }) {
  ...
  const handleAddToCart = async () => {
    setAdding(true);
    await onAddToCart(drug, quantity);
    setTimeout(() => {
      setAdding(false);
      onClose();
    }, 600);
      ...
      <div ref={modalRef} className="bg-white dark:bg-[#1e293b] rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="relative w-full h-44 bg-blue-50 dark:bg-[#0f172a] rounded-t-2xl flex items-center justify-center">
          ...
          <div className="absolute bottom-3 left-3">
            <span className={`text-xs px-3 py-1 rounded-full font-semibold ${drug.is_available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 dark:bg-[#0f172a] rounded-xl p-4 flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-semibold text-gray-900 dark:text-white">{drug.stock} units</p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
              {drug.description || `${drug.name} is a pharmaceutical product classified under ${drug.category}. Always consult your doctor or pharmacist before use.`}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="bg-gray-50 dark:bg-[#0f172a] rounded-lg p-3">
              <p className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
                {drug.dosage || "Consult your doctor or pharmacist for dosage information."}
              </p>
            </div>
            {drug.manufacturer && (
              <div className="flex items-start gap-3 bg-gray-50 dark:bg-[#0f172a] rounded-lg p-3 mt-2">
                <span className="text-xs font-semibold w-24 shrink-0" style={{ color: "#00cfff" }}>Manufacturer</span>
                <span className="text-xs text-gray-600 dark:text-gray-400">{drug.manufacturer}</span>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => setQuantity((q) => Math.min(drug.stock || 99, q + 1))}
          ...
          <button
            ...
            disabled={!drug.is_available || adding}
            className={`w-full py-3 rounded-xl text-white font-semibold text-sm transition flex items-center justify-center gap-2 disabled:opacity-50 ${adding ? "bg-green-600" : "bg-blue-600 hover:bg-blue-700"}`}
          >
            <ShoppingCart size={16} />
            {adding ? "Added!" : !drug.is_available ? "Out of Stock" : `Add ${quantity} to Cart`}
          </button>
